...
## TEMPORARY — debugging workflow (remove when schema sweep is done)

While we are testing/debugging data quality across the fixture schemas, ALWAYS follow this loop for
each schema:

1. Run, picking the dialect from the filename (`*.mysql.sql` → mysql, `*.postgres.sql` → postgresql, etc.):
   `bun run dev generate --schema test/fixtures/schemas/<file> --out test/fixtures/test_outputs/<same-name>`
2. **Wait for the run to finish** (LLM call can take a while — do not abandon it).
3. **Read the generated output file** and judge whether the data is accurate/realistic and would
   insert cleanly (correct types, valid enums, real FKs, sane datetimes, no lorem in structured columns).
4. If anything is wrong, fix it, rebuild, and re-run the same schema until the data is right.

When a fix is **general to the tool** (not specific to one schema's column names), apply it across
ALL affected schema types, not just the one you are testing:
- Engine / prompt / plan-coercion fixes are shared automatically — no extra work.
- **Parser fixes are NOT shared.** If you fix a type-mapping or extraction bug in one parser, check
  and apply the equivalent fix to the other parsers where applicable: `src/parsers/sql.ts`
  (postgresql + mysql + sqlite dialects), `src/parsers/typeorm.ts`, `src/parsers/prisma.ts`.
  Example: the `enum`/`text-variant` handling added for MySQL SQL must also be verified for
  Postgres DDL and TypeORM entities.
...
### MySQL inline `enum(...)` / `set(...)` not extracted (SQL parser)
Inline `ENUM('a','b')` / `SET(...)` columns mapped to `unknown`, so the LLM got no member list and
produced random text for every status/type/gender column (16 warnings on `hr-payroll.mysql.sql`).
- AST shape (node-sql-parser, MySQL): `{ dataType: "ENUM", expr: { type: "expr_list",
  value: [{ value: "male" }, ...] } }`. Members live in `dt.expr.value[].value`.
- Fix in `src/parsers/sql.ts`: `extractEnumMembers(dt)` pulls the members; when present, sets
  `kind = "enum"` and `col.enumValues`. `SET` is treated as an enum (a single member is a valid SET
  value). Downstream `defaultStrategyForColumn` already turns `kind: "enum"` + `enumValues` into an
  enum strategy.
- Spatial types (`point`, `geometry`, `linestring`, `polygon`, …) now map to `string` to stop the
  "unrecognized type" warning. NOTE: this is an approximation — generated values are NOT valid WKT
  spatial SQL, so a real `POINT` column may still reject the insert. True spatial generation is out
  of scope for v1.
